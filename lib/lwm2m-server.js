...
function start(serverConfig, startCallback) {
    ...
    {
        logger.info(context, 'start with preset registry');
    ...
    async.waterfall([
        db.configureDb,
        apply(registry.init,config.getConfig()),
...
function setRegistry(newRegistry) {
    return config.setRegistry(newRegistry);
}
...
function setHandler(serverInfo, type, handler) {
    ...
    if (type === 'unregistration') {
        registry.checkLifetime(config.getConfig().lifetimeCheckInterval, handler);
    }
}
