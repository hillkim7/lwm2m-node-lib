 ...
 *
 * @param {string} deviceId         ID of the device, provided by the LWM2M server.
 ...
 * device.
 *
 * @param {string} deviceId             Internal ID of the device of the subscription (not the endpoint ID).
...
function ResourceListener(deviceId, objectType, objectId, resourceId, observeStream, handler) {
    return {
        ...
        dataHandler: function (chunk) {
            logger.debug(context, 'New data on resource /%s/%s/%s in device [%s]',
        ...
        finishHandler: function(chunk) {
            logger.debug(context, 'Finished observing value of resource /%s/%s/%s in device [%s]',
                objectType, objectId, resourceId, deviceId);
...
function processStream(deviceId, objectType, objectId, resourceId, handler, observeStream, callback) {
    ...
    observeStream.on('data', function (chunk) {
        logger.debug(context, 'Got first piece of data on resource /%s/%s/%s in device [%s]',
            objectType, objectId, resourceId, deviceId);
...
function observe(deviceId, objectType, objectId, resourceId, handler, callback) {
    ...
    logger.debug(context, 'Observing value from resource /%s/%s/%s in device [%s]',
        objectType, objectId, resourceId, deviceId);
